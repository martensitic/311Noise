...
rm(args, caturl, token, url, dl, boros)
```
...
For this capstone project in the HarvardX [Data Science (PH125x) course series](https://www.edx.org/professional-certificate/harvardx-data-science), I examine over nine years of New York City noise complaint records. A mash-up with neighborhood rent allows comparison on a local level, with sufficient breadth to train a machine learning model for projection of future complaints.

## Data Overview

New York City has a 311 telphone hotline for access to city services, enhanced in recent years with web and app portals. One of its many functions is to collect and assign noise complaints to city agencies for enforcement.[^2] As part of its Open Data initiative, the city provides access to the [311 database from 2010 to the present](https://data.cityofnewyork.us/Social-Services/311-Service-Requests-from-2010-to-Present/erm2-nwe9), updated nightly.  Each call (or online submission) is logged with the time, nature of the complaint, and geovalidated location information on the site of the issue.
...
```{r showcdmap, include=TRUE, echo=FALSE, warning=FALSE, fig.cap="New York City's 59 community districts.", cache=FALSE, fig.width=5, fig.height=5}
cdMap
```
...
Since local neighborhood boundaries are rather colloquially defined[^3], and are in any case not included with the Streeteasy figures, I had to manually match neighborhoods to the larger community districts.

[^3]: Not least by those in the real estate business, who often smear the desirable-neighborhood lipstick onto a pig of an apartment.
...
```{r showheatmaps, echo=FALSE, cache=FALSE, include=TRUE, fig.fullwidth=TRUE, fig.height=5, fig.width=10, fig.cap="Complaint density in 2015 and 2018, highlighting Queens CD8."}
heatMap
...
top2 %>% 
  knitr::kable(digits = c(4,4,0,0), col.names = c("Longitude","Latitude", "Complaints","% of Total"),
               caption = "The top two complaint targets in Queens CD8, 2018.")
```

...and those complaints had overwhelmingly to do with alarms and horns.
...
dfNoise %>% filter(year(created_date) == 2018 & community_board == "08 QUEENS") %>%
  ...
  knitr::kable(col.names = c("Descriptor","Complaints"),
               caption = "The complaints about the top two targets.")
```

`r margin_note("A common trajectory: the noise was always there, but new neighbors have different expectations.  There is unfortunately no way to extract whether this example describes new neighbors or old ones---nor many people complaining a little or one person complaining a lot.")`The first location, it turns out, is the EMS ambulance garage at Queens General Hospital.  The second is an FDNY firehouse.  Both were active long before 2018, or 2015, or in the case of the firehouse, 1928, so it's possible that one or more people simply started having a bigger problem with the sirens.

## Noise Complaints, in Aggregate

To advance the analysis, I folded the data into a total number of noise complaints for each month in each community district.  Because the districts vary, I normalized this into a complaint rate per 1000 residents based on the 2010 census counts.  For a bigger picture, we can also look to the monthly rate on a borough and citywide basis.

The borough-level monthly complaint rate shows three immediate trends: a distinct seasonality within each year across all boroughs, a consistent ordering to complaint rates among the five boroughs, and a general increase in complaints with time.

```{r gennbm, include=FALSE}
#####
...
```{r showchan, include=TRUE, echo=FALSE, fig.cap="Monthly complaints by channel: folks that disavow the phone when hiring a car or ordering pad thai probably complain the same way.", cache=FALSE, fig.width=3.5, fig.height=3.5, fig.margin=TRUE}
chan
```
...
Rolling in the median rent benchmark first requires some conversion from the neighborhood-level figures to wider community districts.  As an approximation, I assigned each district the average of the rents for its constituent neighborhoods, as listed at the [NYC Community Portal](https://communityprofiles.planning.nyc.gov/). 
...
```{r showrent, include=TRUE, echo=FALSE, fig.cap="Monthly median rent by borough.", cache=FALSE, fig.width=5, fig.height=5}
boroRent
```

## Complaint Rate vs. Rent

Having assembled a complaint rate and a median rent benchmark for each month across every community district, I started by looking for a relationship between the two.  Pulling back to the borough level for a bigger picture, a scatter of all nine years of complaint/rent points begins to show some trends.

```{r genborosct}
boroScatter<- noiseBoroMonth %>% na.omit() %>%
  ggplot(aes(x=rent, y = rate, color=factor(borough))) + 
  geom_point() + 
  xlab("Median Rent ($)") + ylab("Complaints per 1000 people") + 
  theme(legend.position = "bottom", legend.text = element_text(size = 8),
        legend.title = element_blank()) +
  scale_color_manual(values = boropal)
```

```{r showborosct, include=TRUE, echo=FALSE, fig.cap="Noise complaint rate vs. monthly median rent.", cache=FALSE, fig.width=10, fig.height=5, fig.fullwidth=TRUE}
boroScatter
```

While there are differences between the boroughs, a positive (but not exactly linear) correlation is apparent between complaint rate and rent within each borough.  While this seems intuitive---higher rents may reduce tolerance for noise, while simultaneously drawing the noisy nightlife and crowds that flock to up-and-coming neighborhoods---it's also a case of correlation not implying causation.  As discussed above, both the complaint rate and the rents show seasonality and growth with time that may or may not stem from the same causes.

Despite this apparent relationship bewteen complaints and rent in the wider borough view, will the same hold when filtered down to individual community districts? Taking Manhattan as an example, it does.
...
```{r showmansct, include=TRUE, echo=FALSE, fig.cap="Noise complaint rate vs. monthly median rent in Manhattan's 12 community districts.", cache=FALSE, fig.width=10, fig.height=5, fig.fullwidth=TRUE}
manScat
```

Again, while each individual district shows its own slope, there is a general positive correlation between monthly rent and monthly noise complaint rate.[^8]  The remaining boroughs show similar trends, though at a narrower range of rents.

[^8]: This slope describes a personality of sorts. Inwood, CD12 on the left? Sensible but complains a lot. Midtown, CD5 on the right? Extravagant and just as prone. Upper East Side, CD8 in the middle? Actually pretty laid back.
...
```{r showworst, include=TRUE, echo=FALSE, fig.cap="Maximum monthly rent and complaint rate for each district, 2018.", cache=FALSE, fig.width=10, fig.height=5, fig.fullwidth=TRUE}
scatter2018
```
...
Of course, we can't have nice things here for very long, and a good value very quickly becomes the same old grind once people find out and move there.[^12]  A model for predicing future noise complaint rates could prove pretty useful for the noise-sensitive and rent-averse, not to mention for the city agencies tasked with responding to noise complaints.
